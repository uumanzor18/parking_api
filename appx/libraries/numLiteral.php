...
class numLiteral {
	...
	private $mMonedaSng = "";
	private $mMonedaPl = "";
	private $mCentavoLetra = true;
	
	function __construct($monedaSng, $monedaPl, $ctvsLetras=true) {
		$this->mMonedaSng = $monedaSng;
		$this->mMonedaPl = $monedaPl;
		$this->mCentavoLetra = $ctvsLetras;
	}
	...
	public function convertir($numero){ 
		...
		if ($this->mCentavoLetra) {
			if ($tempnum[1] == "" || $tempnum[1] >= 100) { 
				...
				$TextEnd .= " CON ".$this->decena($tempnum[1]) ; 
				$TextEnd .= " CENTAVOS ";
			}
		}
		else {// COLOCA CENTAVOS COMO N/100
			if ($tempnum[1] == "" || $tempnum[1] >= 100) { 
				$tempnum[1] = "00" ; 
			} 
			$TextEnd .= " CON ".$tempnum[1] ; 
			$TextEnd .= "/100 "; 
		}
